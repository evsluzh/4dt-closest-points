...
{
    int index = find_edge_by_time(t);
    if (index == -1)
    {
        return false;
    }
    return edge(index)->get_point(t, p);
}
...
int Route::find_edge_by_time(double t) const
{
    int l = 0, r = size();
    while (l < r)
    {
    ...
    assert(t <= m_edges[r]->b()->t());
    if (r == 0 && t < m_edges[0]->a()->t())
    {
        return -1;
    }
    assert(m_edges[r]->a()->t() <= t);
    return r;
}
